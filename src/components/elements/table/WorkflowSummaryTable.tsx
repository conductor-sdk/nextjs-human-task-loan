...
type ValRendererSorter = {
  renderer: (n: WorkflowSummary) => ReactNode;
  sortId?: keyof WorkflowSummary;
};
...
interface EnhancedTableProps {
  onRequestSort: (
    ...
    property: keyof WorkflowSummary
  ) => void;
  order: Order;
  orderBy: string;
...
function EnhancedTableHead(props: EnhancedTableProps) {
  ...
  const createSortHandler =
    (property: keyof WorkflowSummary) => (event: React.MouseEvent<unknown>) => {
...
export const WorkflowSummaryTable = ({ columns, executions }: Props) => {
  const [order, setOrder] = useState<Order>("asc");
  const [orderBy, setOrderBy] = useState<keyof WorkflowSummary>("workflowId");
  ...
  const visibleRows = useMemo(
    () => executions.sort(getComparator(order, orderBy)),
    [order, orderBy, executions]
  );
  return (
